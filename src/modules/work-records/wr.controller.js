...
const service = require('./wr.service');
const { buildImagePath } = require('../../utils/file.helper');

const UPLOAD_SUBPATH = 'wr';

// ── Employee ─────────────────────────────────────────────────

const employeeList = asyncHandler(async (req, res) => {
  const { page, limit, offset } = pagination.parse(req.query);
  const { status, search, lt9 } = req.query;
  const { data, total } = await service.listRecords({ status, search, searchLt9: lt9, limit, offset });
...
const employeeCreate = asyncHandler(async (req, res) => {
  ...
  const fields = { ...req.body, lt1: req.user.name };
