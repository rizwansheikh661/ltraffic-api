...
const TABLE = LEGACY.TFL;
const DOC_TABLE = LEGACY.UPLOAD_TFL;

const COLS = 'id, jobstatus, assignedto, customer, jobnumber, sitereference, location, postcode, area, workstream, permitstatus, startdate, enddate, notes';

const INACTIVE_STATUSES = ['Pending', 'Delayed', 'Completed', 'Cancelled', 'Awaiting Invoicing', 'Invoiced', 'Closed'];
...
async function findActive({ search, searchAssignedto, limit, offset } = {}, conn = pool) {
  ...
  if (search) {
    conditions.push('jobnumber LIKE :search');
...
async function create(fields, conn = pool) {
  const [result] = await conn.query(
    `INSERT INTO ${TABLE}
       (jobstatus, assignedto, customer, jobnumber, sitereference, location, postcode, area, workstream, permitstatus, startdate, enddate, notes)
     VALUES (:jobstatus, :assignedto, :customer, :jobnumber, :sitereference, :location, :postcode, :area, :workstream, :permitstatus, :startdate, :enddate, :notes)`,
    {
...
async function update(id, fields, conn = pool) {
  const allowed = [
    'jobstatus', 'assignedto', 'customer', 'jobnumber', 'sitereference',
    'location', 'postcode', 'area', 'workstream', 'permitstatus',
