...
const repo = require('./tfl.repository');
...
async function getJobById(id) {
  ...
  if (!row) throw ApiError.notFound('TFL job not found');
  return formatJob(row);
}

async function getDocuments(jobId, query) {
  const job = await repo.findById(jobId);
  if (!job) throw ApiError.notFound('TFL job not found');
...
async function updateJob(id, fields) {
  ...
  if (!existing) throw ApiError.notFound('TFL job not found');
...
async function removeJob(id) {
  ...
  if (!deleted) throw ApiError.notFound('TFL job not found');
}
