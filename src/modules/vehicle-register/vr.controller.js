...
const service = require('./vr.service');

const UPLOAD_SUBPATH = 'vrfiles';

const list = asyncHandler(async (req, res) => {
  const { page, limit, offset } = pagination.parse(req.query);
  const { search } = req.query;
  const { data, total } = await service.getAll({ search, archived: false, limit, offset });
...
const listArchived = asyncHandler(async (req, res) => {
  const { page, limit, offset } = pagination.parse(req.query);
  const { search } = req.query;
  const { data, total } = await service.getAll({ search, archived: true, limit, offset });
  return ok(res, data, pagination.meta({ page, limit }, total));
});
...
const listDocuments = asyncHandler(async (req, res) => {
  const { page, limit, offset } = pagination.parse(req.query);
  const { search } = req.query;
  const { rows, total } = await service.getDocuments({ vrId: req.params.id, search, limit, offset });
